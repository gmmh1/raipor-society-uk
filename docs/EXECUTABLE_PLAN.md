...
- Events QR check-in completed (was previously generated but unused — `EventCheckInView` now accepts `qr_token` as well as `registration_id`), waitlisting when capacity is reached, and self-cancellation with automatic oldest-waitlisted promotion. See ADR 0010. Notifications real push/WhatsApp adapters and Shop↔Finance payment integration remain outstanding — split out of this phase as separate follow-up slices (see ADR 0010's Follow-up section).
